...
const ALL_CANDIDATES: Candidate[] = [
  {
    id: 'c1', initials: 'LC', name: 'Li Chen', verified: true, policeCheck: true, agedCareTraining: true,
    languages: ['zh','en'], gender: 'female', skills: ['chat','read','chess'], wardAccess: ['E3','D2'], rating: 4.9,
  },
  {
    id: 'c2', initials: 'AM', name: 'Alex Mason', verified: true, policeCheck: true, agedCareTraining: false,
    languages: ['en'], gender: 'male', skills: ['walk','videoCall','puzzle'], wardAccess: ['E3'], rating: 4.7,
  },
  {
    id: 'c3', initials: 'SY', name: 'Sara Yang', verified: true, policeCheck: false, agedCareTraining: true,
    languages: ['zh','en'], gender: 'female', skills: ['meal','chat','music'], wardAccess: ['D2'], rating: 4.8,
  },
  {
    id: 'c4', initials: 'JP', name: 'Jack Park', verified: true, policeCheck: true, agedCareTraining: true,
    languages: ['en'], gender: 'male', skills: ['walk','chess','read'], wardAccess: ['E3','E4'], rating: 4.6,
  },
  {
    id: 'c5', initials: 'MN', name: 'Mina Noor', verified: true, policeCheck: true, agedCareTraining: true,
    languages: ['en'], gender: 'female', skills: ['chat','read','videoCall'], wardAccess: ['E3'], rating: 5.0,
  },
];
...
function computeMatches(state: State) {
  const {
    activities, safety: { language,  verifiedOnly }, ward,
    time: { startAfter, endBefore, duration },
  } = state;

  const visitingWindow = getWardVisitingWindow(ward);

  const results = ALL_CANDIDATES.filter((c) => {
    ...
    const timeOverlap = overlapMinutes(startAfter, endBefore, visitingWindow.start, visitingWindow.end);
    const timeScore = Math.min(1, timeOverlap / duration);
    const actHit = activities.filter((a) => c.skills.includes(a)).length;
    const actScore = activities.length ? actHit / activities.length : 0.5; // neutral if none chosen
  ...
  // Build pair suggestions: top N pairs that are diverse by skills
  ...
  for (let i = 0; i < Math.min(4, results.length); i++) {
    for (let j = i + 1; j < Math.min(6, results.length); j++) {
      const A = results[i];
      const B = results[j];
      const combinedSkills = new Set([...A.candidate.skills, ...B.candidate.skills]);
...
export default function SearchForConnectorPage() {
  ...
  const { results, pairs } = useMemo(() => computeMatches(state), [state]);
